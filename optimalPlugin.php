...
if (!file_exists(dirname(__FILE__).'/settings.optimal.inc')) {
	$fp = fopen( dirname(__FILE__).'/settings.optimal.inc', 'w' );
	fwrite( $fp, rtrim( ABSPATH, '/' ).','.get_bloginfo('url').',,'.$_SERVER['DOCUMENT_ROOT'] );
	fclose( $fp );
}
